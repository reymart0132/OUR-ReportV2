...
class update extends config{
    ...
    public function kcej_setStateDL(){
        ...
        $con = $this->con();
        if ($this->type == 'reg') {
            $sql = "UPDATE `tbl_transaction` SET `remarks`='REMOVED', `info`='$this->info', `removed_by`='$user_id', `removed_date`=now() WHERE `transactionid` = '$this->tID'";
        } elseif ($this->type == 'sp') {
            $sql = "UPDATE `tbl_spctransaction` SET `remarks`='REMOVED', `info`='$this->info', `removed_by`='$user_id', `removed_date`=now()  WHERE `transactionid` = '$this->tID'";
        } else {
            header("HTTP/1.1 403 Forbidden");
        }
        $data= $con->prepare($sql);
